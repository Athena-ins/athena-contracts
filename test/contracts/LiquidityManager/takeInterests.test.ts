...
export function LiquidityManager_takeInterests() {
  context("takeInterests", function () {
    before(async function () {
      this.args = {};
    });

    it("should revert if called by someone other than the position owner", async function () {
    ...
    it("should succeed if called by the position owner", async function () {
    ...
    it("should correctly update the position's capital and strategy rewards", async function () {
      const owner = this.signers.account1;
      const positionId = this.args.positionId;  
    ...
    it("should revert if there's a committed withdrawal for the position", async function () {
      const owner = this.signers.account1;
    ...
    it("should correctly calculate new user capital and strategy rewards", async function () {
      const owner = this.signers.account1;
      const positionId = this.args.positionId;  
    ...
    it("should correctly update the position's reward index", async function () {
      const owner = this.signers.account1;
      const positionId = this.args.positionId;  
    ...
    it("should withdraw interests from the strategy manager", async function () {
      const owner = this.signers.account1;
      const positionId = this.args.positionId;
    ...
    it("should update the position's supplied capital", async function () {
      const owner = this.signers.account1;
      const positionId = this.args.positionId;
    ...
    it("should interact correctly with the position token contract to get the owner of the position", async function () {
      const positionId = this.args.positionId;
    ...
    it("should interact correctly with the strategy manager for withdrawing rewards", async function () {
      const positionId = this.args.positionId;
      const poolId = this.args.poolId;
      const strategyId = (
        await this.contracts.LiquidityManager.poolInfo(poolId)
    ...
    it("should interact correctly with the virtual pool for taking pool interests", async function () {
      const positionId = this.args.positionId;
    ...
    it("should handle cases where the position is involved in multiple pools", async function () {
      const positionId = this.args.positionId;
      const poolIds = this.args.poolIds;
      for (let i = 0; i < poolIds.length; i++) {
        expect( await this.contracts.LiquidityManager.takeInterests(positionId))
    ...
    it("should correctly handle the yield bonus", async function () {
      const positionId = this.args.positionId;
