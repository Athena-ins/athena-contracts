...
contract Athena is IAthena, ReentrancyGuard, Ownable {
  ...
  function takeStakingProfits() external {
    uint256 amountRewards = stakedAtensGPInterface.claimRewards(
      msg.sender
    );
  ...
  function buyPolicies(
    ...
    for (uint256 i = 0; i < nbPolicies; i++) {
      ...
      if (0 < _atensLocked) {
        ...
        stakedAtensPoInterface.createStakingPosition(
          coverId,
          _atensLocked
        );
      }
  ...
  function withdrawCoverRefundStakedAten(
    ...
    stakedAtensPoInterface.withdrawStakedAten(
      coverId_,
      amount_,
      msg.sender
    );
  ...
  function withdrawCoverRefundRewards(
    ...
    uint256 netRewards = stakedAtensPoInterface.withdrawRewards(
      policyId_
    );
    if (0 < netRewards) {
      atensVaultInterface.sendCoverRefundReward(
        msg.sender,
        netRewards
      );
    }
