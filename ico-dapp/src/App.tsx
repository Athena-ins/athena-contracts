...
const wei = BigNumber.from(10).pow(18);
  ...
  function App() {
    ...
    const { value: ethPrice, error: ethPriceError } =
      useCall({
        contract: new Contract(ATHENA_ICO_CONTRACT_ADDRESS[chainId] as string, abi),
        method: "getLatestPrice",
        args: [],
      }) ?? {};
    const { value: atenToken, error: atenTokenError } =
      useCall({
        contract: new Contract(ATHENA_ICO_CONTRACT_ADDRESS[chainId], abi),
        method: "aten",
        args: [],
      }) ?? {};
    ...
    const handleApprove = async (e: any) => {
      e.preventDefault();
      try {
        const usdtContract = new ethers.Contract(USDT[chainId], [
          {
            inputs: [
