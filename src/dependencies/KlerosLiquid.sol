...
import { SortitionSumTreeFactory } from "./libs/SortitionSumTreeFactory.sol";
...
contract KlerosLiquid is TokenController, Arbitrator {
  ...
  SortitionSumTreeFactory.SortitionSumTrees
    internal sortitionSumTrees; // The sortition sum trees.
   ...
   */
  function changeSubcourtMinStake(
    uint96 _subcourtID,
    uint _minStake
  ) external onlyByGovernor {
    require(
      _subcourtID == 0 ||
        courts[courts[_subcourtID].parent].minStake <= _minStake
    );
   ...
   */
  function changeSubcourtJurorFee(
    uint96 _subcourtID,
    uint _feeForJuror
  ) external onlyByGovernor {
   ...
   */
  function changeSubcourtJurorsForJump(
    uint96 _subcourtID,
    uint _jurorsForCourtJump
  ) external onlyByGovernor {
  ...
  function passPhase() external {
    if (phase == Phase.staking) {
      require(
        now - lastPhaseChange >= minStakingTime,
      ...
      require(
        disputesWithoutJurors == 0 ||
          now - lastPhaseChange >= maxDrawingTime,
    ...
    if (dispute.period == Period.evidence) {
      require(
        dispute.votes.length > 1 ||
          ...
          courts[dispute.subcourtID].timesPerPeriod[
            uint(dispute.period)
          ],
   ...
   */
  function executeDelayedSetStakes(
    uint _iterations
  ) external onlyDuringPhase(Phase.staking) {
    uint actualIterations = (nextDelayedSetStake + _iterations) - 1 >
      lastDelayedSetStake
      ? (lastDelayedSetStake - nextDelayedSetStake) + 1
      : _iterations;
    uint newNextDelayedSetStake = nextDelayedSetStake +
      actualIterations;
    require(newNextDelayedSetStake >= nextDelayedSetStake);
    for (
      uint i = nextDelayedSetStake;
      i < newNextDelayedSetStake;
      i++
    ) {
  ...
  function drawJurors(
    ...
    external
    onlyDuringPhase(Phase.drawing)
    onlyDuringPeriod(_disputeID, Period.evidence)
  {
    Dispute storage dispute = disputes[_disputeID];
    ...
    for (uint i = dispute.drawsInRound; i < endIndex; i++) {
      ...
      (
        ...
        uint subcourtID
      ) = stakePathIDToAccountAndSubcourtID(
          sortitionSumTrees.draw(
            bytes32(dispute.subcourtID),
            uint(keccak256(RN, _disputeID, i))
          )
        );

      // Save the vote.
      dispute
      .votes[dispute.votes.length - 1][i].account = drawnAddress;
      jurors[drawnAddress].lockedTokens += dispute
      ...
      if (i == dispute.votes[dispute.votes.length - 1].length - 1)
  ...
  function castCommit(
    ...
    for (uint i = 0; i < _voteIDs.length; i++) {
      require(
        dispute
        .votes[dispute.votes.length - 1][_voteIDs[i]].account ==
          msg.sender,
        "The caller has to own the vote."
      );
      require(
        dispute.votes[dispute.votes.length - 1][_voteIDs[i]].commit ==
          bytes32(0),
        "Already committed this vote."
      );
      dispute
      .votes[dispute.votes.length - 1][_voteIDs[i]].commit = _commit;
    }
  ...
  function castVote(
    ...
    for (uint i = 0; i < _voteIDs.length; i++) {
      ...
      require(
        !courts[dispute.subcourtID].hiddenVotes ||
          dispute
          .votes[dispute.votes.length - 1][_voteIDs[i]].commit ==
          keccak256(_choice, _salt),
        "The commit must match the choice in subcourts with hidden votes."
      );
      require(
        !dispute.votes[dispute.votes.length - 1][_voteIDs[i]].voted,
        "Vote already cast."
      );
      dispute
      .votes[dispute.votes.length - 1][_voteIDs[i]].choice = _choice;
      dispute
      .votes[dispute.votes.length - 1][_voteIDs[i]].voted = true;
    }
    dispute.votesInEachRound[dispute.votes.length - 1] += _voteIDs
      .length;

    // Update winning choice.
    VoteCounter storage voteCounter = dispute.voteCounters[
      dispute.voteCounters.length - 1
    ];
    ...
    if (_choice == voteCounter.winningChoice) {
      ...
      if (
        ...
        voteCounter.counts[voteCounter.winningChoice]
      ) {
        // Tie.
  ...
  function computeTokenAndETHRewards(
    ...
    if (dispute.voteCounters[dispute.voteCounters.length - 1].tied) {
      ...
      if (activeCount > 0) {
        tokenReward =
          dispute.penaltiesInEachRound[_appeal] /
          activeCount;
      ...
      uint winningChoice = dispute
        .voteCounters[dispute.voteCounters.length - 1]
        .winningChoice;
      uint coherentCount = dispute.voteCounters[_appeal].counts[
        winningChoice
      ];
      tokenReward =
        dispute.penaltiesInEachRound[_appeal] /
  ...
  function execute(
    ...
    if (
      ...
      if (end > dispute.votes[_appeal].length)
        end = dispute.votes[_appeal].length;
    } else {
      ...
      (tokenReward, ETHReward) = dispute.repartitionsInEachRound[
        _appeal
      ] >= dispute.votes[_appeal].length
        ? computeTokenAndETHRewards(_disputeID, _appeal)
        : (0, 0); // Compute rewards if rewarding.
      if (end > dispute.votes[_appeal].length * 2)
        end = dispute.votes[_appeal].length * 2;
    }
    for (
      uint i = dispute.repartitionsInEachRound[_appeal];
      i < end;
      i++
    ) {
      Vote storage vote = dispute.votes[_appeal][
        i % dispute.votes[_appeal].length
      ];
      if (
        vote.voted &&
        (vote.choice ==
        ...
        if (i >= dispute.votes[_appeal].length) {
          ...
          jurors[vote.account].lockedTokens -= dispute
            .tokensAtStakePerJuror[_appeal];
        }
      } else {
        ...
        if (i < dispute.votes[_appeal].length) {
          ...
          // Penalize.
          uint penalty = dispute.tokensAtStakePerJuror[_appeal] >
          ...
          if (
            pinakion.balanceOf(vote.account) <
            jurors[vote.account].stakedTokens ||
            !vote.voted
          )
            for (
              uint j = 0;
              j < jurors[vote.account].subcourtIDs.length;
              j++
            )
              _setStake(
                vote.account,
                jurors[vote.account].subcourtIDs[j],
                0
              );
        }
      }
      if (i == dispute.votes[_appeal].length - 1) {
        ...
        if (
          ...
          (tokenReward, ETHReward) = computeTokenAndETHRewards(
            _disputeID,
            _appeal
          );
        }
    ...
    returns (uint disputeID)
  {
    ...
    dispute.votes[dispute.votes.length++].length =
      msg.value /
      courts[dispute.subcourtID].feeForJuror;
    dispute.voteCounters[dispute.voteCounters.length++].tied = true;
    dispute.tokensAtStakePerJuror.push(
      (courts[dispute.subcourtID].minStake *
        courts[dispute.subcourtID].alpha) / ALPHA_DIVISOR
    );
    ...
    onlyDuringPeriod(_disputeID, Period.appeal)
  {
    ...
    if (
      dispute.votes[dispute.votes.length - 1].length >=
      courts[dispute.subcourtID].jurorsForCourtJump
    )
  ...
  function proxyPayment(
    address _owner
  ) public payable returns (bool allowed) {
    allowed = false;
  }
  ...
  function onTransfer(
    ...
    if (lockInsolventTransfers) {
      ...
      if (
        newBalance < jurors[_from].stakedTokens ||
        newBalance < jurors[_from].lockedTokens
      ) return false;
    }
    allowed = true;
  }
  ...
  function appealCost(
    ...
    Dispute storage dispute = disputes[_disputeID];
    uint lastNumberOfJurors = dispute
      .votes[dispute.votes.length - 1]
      .length;
    if (
      lastNumberOfJurors >=
      courts[dispute.subcourtID].jurorsForCourtJump
      ...
      if (dispute.subcourtID == 0)
        ...
        cost =
          courts[courts[dispute.subcourtID].parent].feeForJuror *
  ...
  function currentRuling(
    uint _disputeID
  ) public view returns (uint ruling) {
    Dispute storage dispute = disputes[_disputeID];
    ruling = dispute
      .voteCounters[dispute.voteCounters.length - 1]
      .tied
  ...
  function _setStake(
    ...
    bytes32 stakePathID = accountAndSubcourtIDToStakePathID(
      _account,
      _subcourtID
    );
    uint currentStake = sortitionSumTrees.stakeOf(
      bytes32(_subcourtID),
      stakePathID
    );
    if (
      !(_stake == 0 ||
        currentStake > 0 ||
        juror.subcourtIDs.length < MAX_STAKE_PATHS)
    ...
    if (_stake == 0) {
      for (uint i = 0; i < juror.subcourtIDs.length; i++)
        if (juror.subcourtIDs[i] == _subcourtID) {
          juror.subcourtIDs[i] = juror.subcourtIDs[
            juror.subcourtIDs.length - 1
          ];
  ...
  function getSubcourt(
    uint96 _subcourtID
  ) external view returns (uint[] children, uint[4] timesPerPeriod) {
  ...
  function getVote(
    ...
    external
    view
    returns (address account, bytes32 commit, uint choice, bool voted)
  {
  ...
  function getVoteCounter(
    ...
    external
    view
    returns (uint winningChoice, uint[] counts, bool tied)
  {
    Dispute storage dispute = disputes[_disputeID];
    ...
    for (uint i = 0; i <= dispute.numberOfChoices; i++)
      counts[i] = voteCounter.counts[i];
    ...
    )
  {
    ...
    for (uint i = 0; i < dispute.votes.length; i++)
      votesLengths[i] = dispute.votes[i].length;
  ...
  function getJuror(
    address _account
  ) external view returns (uint96[] subcourtIDs) {
    Juror storage juror = jurors[_account];
